...
class Im2LatexDataset:
    ...
    def __len__(self):
        return self.ext_size if self.ext_size != -1 else self.size
    ...
    def _get_ext_size(self):
        """update external size which has to be used in pt-lightning"""
        self.ext_size = 0
        for _ in tqdm(self, desc="iterating through dataset to get actual size", total=self.size):
            self.ext_size += 1
